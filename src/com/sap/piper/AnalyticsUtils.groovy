...
class AnalyticsUtils {
    ...
    void registerEventListener(String eventType, Closure listener){
    ...
    void notifyAndPushToSWA(Map parameters, Map config){
        if(!parameters.eventType){
            parameters.eventType = 'library-os'
        }
        def eventType = parameters.eventType

        if(eventListeners[eventType]){
            def listeners = eventListeners[eventType]
            for(int i=0; i<=listeners.size();i++){
                listeners[i](parameters)
            }
        }

        this.pushToPiperOsSWA(parameters, config)
    }

    private pushToPiperOsSWA(Map parameters, Map config) {
//        try {
